...
export class QuizService {
  ...
  getQuestionsByQuiz(quizId:string): Observable<any> {
    return this.http.get(this.api_question + "/" + quizId).pipe(
      catchError(throwError)
    );
  }
  updateQuestion(quizId:string, payload : any): Observable<any> {
    return this.http.put(this.api_question + "/" + quizId,payload).pipe(
      catchError(throwError)
    );
  }
